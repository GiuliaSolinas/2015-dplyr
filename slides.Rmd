...
# top_n(flights, 10) (wählt Top 10-Werte, gegeben Gruppierung und Sortierung)
```
...
Verb | Aufgabe
--- | ---
`filter(flights, Month < 3)` | filtert aus Zeilen
...
<!---
## Auswählen von Variablen: `select` und alle Helper Functions 

|  | |  | 
|  --- | --- | --- | 
|  `Year:DayofMonth` | |  `num_range("x", 1:5, width = 2)` | 
|  `contains("Taxi")` | |  `-TaxiIn` | 
|  `starts_with("Taxi")` | | `one_of(vector)` | 
...
|  | |  |  
|  --- | --- | --- |  
|  `name01:name05` | |  `num_range("name", 1:5, width = 2)` |  
|  `contains("name")` | |  `-TaxiIn` |  
|  `starts_with("name")` | | `one_of(c("name01", name02))` |  
|  `ends_with("01")` | | `everything()` |    
|   `matches(".me0.")` | | |  
--->
...
my_db <- src_postgres(dbname = "testdb", host = "localhost", user = "philipprosch")
...
* `ungroup`
* Standard evaluation: `filter_`, `select_`, ... 
* `data_frame`
